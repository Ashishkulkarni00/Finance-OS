...
| **7** | Insight engine core + 4 rules (shortfall, overdue, card bill due, goal behind) | `insight` package, `FinancialContext` | V19: `insight_state` | `GET /insights`, snooze, dismiss | `InsightList`; Today and Month use it; old components become renderers | Per-rule units; ranking | Behaviour parity with today's warnings first |
